...
- **The pain:** No SQL tools. No SQLite. Nothing. Just me, a text file, and a dream - excepts it's a nightmare. 😥
    - I want to save two customer information then get with format is name - age, writing "Alice - 30", "Bob - 31"? Fine for 2 lines. Try 200. Or 2,000. Parsing it back? Pure torture fopen() and fscanf(), endless loops, parying it doesn't crash 😢.
...
- Slow as hell. Every read/write's a disk slog. Lose a comma? Corrupted mess. Need to find "Bob"? Good look scanning line-by-line like a caveman.
## Preparement
...
- **Hello, World!** First stab at C glory [(My Pull Request)] (https://github.com/LeviathanST/TinySQL/pull/1)
    1. **Makefile Magic:** Tired of typing `gcc main.c -o tinysql && ./tinyql` every time. Makefile = one-word victory: make 🤯. Boom, built and running in oneshot!
    ...
    - It's tough to see early, but next stages, it'll flex its power soon.
- **First Strike: Data, Baby!** [(My Pull request)] (https://github.com/LeviathanST/TinySQL/pull/2)
  - Let me guess, "WTF!" is on your mind after read my pull request - right? I know what is going on, hardcoded "Alice - 30", "Bob - 31" in the code - no text files, just *BAM*, data's there. Funny first win, but it's quicker than reading files!
